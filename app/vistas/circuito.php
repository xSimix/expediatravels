...
if (!empty($gallery)) {
    foreach ($gallery as $image) {
        ...
        $alt = $image['alt'] ?? ($image['descripcion'] ?? $title);
...
$heroSectionStyle = '';
if ($heroStyle !== '' && empty($heroSlides)) {
    $heroSectionStyle = ' style="' . $heroStyle . '"';
}
$summaryParagraphs = [];
...
<body class="page page--detail">
    ...
    <main class="detail-page circuit-page">
        <section class="detail-hero circuit-hero"<?= $heroSectionStyle; ?>>
            <?php if (!empty($heroSlides)): ?>
                <div
                    class="circuit-hero__slider"
                    data-slider="circuit-hero"
                    data-visible-desktop="3"
                    data-visible-tablet="2"
                    data-visible-mobile="1"
                >
                    <div class="circuit-hero__track">
                        <?php foreach ($heroSlides as $slide):
                            $slideSrc = $slide['src'] ?? '';
                            if ($slideSrc === '') {
                                continue;
                            }
                            $slideAlt = $slide['alt'] ?? $title;
                            if (!is_string($slideAlt) || trim($slideAlt) === '') {
                                $slideAlt = $title;
                            }
                        ?>
                            <div class="circuit-hero__slide">
                                <figure>
                                    <img src="<?= htmlspecialchars($slideSrc, ENT_QUOTES); ?>" alt="<?= htmlspecialchars($slideAlt); ?>" loading="lazy" />
                                </figure>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    <div class="circuit-hero__dots" role="tablist" aria-label="Navegación de galería"></div>
                </div>
            <?php endif; ?>
    ...
    <script src="scripts/circuit-hero-slider.js" defer></script>
